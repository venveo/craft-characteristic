...
<template>
  <div class="buttons last add-button">
    <div class="select rounded-r-none">
      <select v-model="selectedCharacteristicIndex" class="rounded-r-none">
        <option :value="index" v-for="(characteristic, index) in characteristics" :key="characteristic.id">
          {{ characteristic.title }}
        </option>
      </select>
    </div>
    <div @click="handleAddBlock(characteristics[selectedCharacteristicIndex])" class="btn add icon rounded-l-none">Add
    </div>
  </div>
</template>
<script lang="ts">
...
export default defineComponent({
  ...
  setup(props) {
    const selectedCharacteristicIndex = <null | number>ref(0)
    ...
    const handleAddBlock = function (characteristic: Characteristic) {
      ...
      this.$emit('addLinkBlock', linkBlock)
    }
    return {
